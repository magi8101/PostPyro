...
class Pool:
    ...
    async def execute(self, query: str, params: Optional[List[Any]] = None) -> int:
        """Runs a statement on a pooled connection and returns rows affected. `bool`/`int`/`float`/`str`/`None`/`datetime`/`date`/`time`/`uuid.UUID`/`Decimal`/`dict`/`bytes`/`bytearray` params bind natively with the right Postgres wire type (naive datetime as TIMESTAMP, tz-aware as TIMESTAMPTZ, dict as JSON); a homogeneous `list`/`tuple` binds as a native Postgres array, a mixed/nested one as JSON; other objects fall back to `str()` as TEXT."""
        ...
    async def query(self, query: str, params: Optional[List[Any]] = None) -> List[Row]:
